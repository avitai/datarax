...
`activate.sh` is checked into the repository (not generated by setup) and loads
`.datarax.env` when sourced.
...
Code in `src/`, `tests/`, `examples/`, `scripts/`, and `benchmarks/` is
type-checked. Certain rules are relaxed to accommodate JAX's dynamic typing patterns.
...
The pipeline runs a number of hooks, including those below. See
[`.pre-commit-config.yaml`](https://github.com/avitai/datarax/blob/main/.pre-commit-config.yaml)
