...
`TFDSSource` provides integration with [TensorFlow Datasets (TFDS)](https://www.tensorflow.org/datasets), giving access to hundreds of ready-to-use datasets with automatic conversion from TensorFlow tensors to JAX arrays.
...
TFDSSource requires TensorFlow and tensorflow-datasets:
...
source = TFDSSource(config, rngs=nnx.Rngs(42))
```
