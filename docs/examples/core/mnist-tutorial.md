...
def create_train_pipeline():
    ...
    source = TFDSSource(train_config, rngs=nnx.Rngs(42, shuffle=42))
...
def create_test_pipeline():
    ...
    source = TFDSSource(test_config, rngs=nnx.Rngs(0))
...
flowchart TB
    subgraph Data["Data Pipeline"]
        TFDS[TFDSSource<br/>MNIST 60k samples]
...
```
Training Pipeline:
TFDSSource → Preprocess → Brightness → Noise → Model

Testing Pipeline:
TFDSSource → Preprocess → Model
```
