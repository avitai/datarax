...
1. Load CIFAR-10 using `TFDSSource` with proper configuration
...
| PyTorch | Datarax |
|---------|---------|
| `datasets.CIFAR10(root, train=True)` | `TFDSSource(TfdsDataSourceConfig(name="cifar10", split="train"))` |
...
| TensorFlow | Datarax |
|------------|---------|
| `tfds.load("cifar10", split="train")` | `TFDSSource(TfdsDataSourceConfig(name="cifar10", split="train"))` |
...
Configure `TFDSSource` to load CIFAR-10 training split. We use a subset for this quick reference to keep runtime short.
...
flowchart LR
    subgraph Source["TFDS Source"]
        T[TFDSSource<br/>CIFAR-10<br/>1000 samples]
    end
...
print("Pipeline: TFDSSource(CIFAR-10) -> Normalize -> Output")
...
**Terminal Output:**
```
Pipeline: TFDSSource(CIFAR-10) -> Normalize -> Output
Batch size: 32
...
- [API Reference: TFDSSource](../../sources/tfds_source.md) - Complete TFDS API documentation
