...
Load and process datasets from [HuggingFace Hub](https://huggingface.co/datasets) using Datarax's `HFSource`. This enables access to thousands of pre-built datasets with seamless integration into your data pipelines.
...
| PyTorch | Datarax |
|---------|---------|
| `datasets.load_dataset("mnist")` | `HFSource(HfDataSourceConfig(name="mnist"))` |
| `dataset["train"]` | `HfDataSourceConfig(split="train")` |
| `IterableDataset` + `DataLoader` | `HFSource` with `streaming=True` |
...
| TensorFlow | Datarax |
|------------|---------|
| `tfds.load("mnist")` | `HFSource(HfDataSourceConfig(name="mnist"))` |
...
flowchart LR
    ...
    subgraph Source["HFSource"]
        CFG[HfDataSourceConfig<br/>streaming=True]
        SRC[HFSource<br/>Load on demand]
    end
...
config = HfDataSourceConfig(name="cifar10", split="train", streaming=True)

# ImageNet-1K: 1.28M images, 1000 classes
config = HfDataSourceConfig(name="imagenet-1k", split="train", streaming=True)

# Fashion-MNIST: 70K 28x28 grayscale fashion items
config = HfDataSourceConfig(name="fashion_mnist", split="train", streaming=True)
```
...
config = HfDataSourceConfig(name="imdb", split="train", streaming=True)

# SQuAD: Reading comprehension dataset
config = HfDataSourceConfig(name="squad", split="train", streaming=True)

# WikiText-103: Language modeling dataset
config = HfDataSourceConfig(name="wikitext", subset="wikitext-103-v1", split="train", streaming=True)
```
...
config = HfDataSourceConfig(name="coco", subset="2017", split="train", streaming=True)

# Conceptual Captions: 3.3M image-text pairs
config = HfDataSourceConfig(name="conceptual_captions", split="train", streaming=True)
```
...
- [API Reference: HFSource](../../../sources/hf_source.md) - Complete HuggingFace API documentation
