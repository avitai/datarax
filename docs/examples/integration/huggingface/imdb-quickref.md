...
1. Load IMDB dataset using `HFSource` with streaming mode
...
| PyTorch | Datarax |
|---------|---------|
| `datasets.load_dataset('imdb', split='train')` | `HFSource(HfDataSourceConfig(name='stanfordnlp/imdb', split='train'))` |
...
| TensorFlow | Datarax |
|------------|---------|
| `tfds.load('imdb_reviews')` | `HFSource(HfDataSourceConfig(name='stanfordnlp/imdb'))` |
...
print("Pipeline: HFSource(IMDB) -> LabelNormalizer -> Output")
print("Note: Text field excluded for batching")
```

**Terminal Output:**
```
Pipeline: HFSource(IMDB) -> LabelNormalizer -> Output
