...
| PyTorch | Datarax |
|---------|---------|
| `datasets.load_dataset('mnist', split='train')` | `HFSource(HfDataSourceConfig(name='mnist', split='train'))` |
| `DataLoader(shuffle=True, num_workers=4)` | `HFSource` with `shuffle=True, shuffle_buffer_size=N` |
...
|--------------------|---------|
| `tfds.load('mnist', split='train')` | `HFSource(HfDataSourceConfig(name='mnist', split='train'))` |
...
exclude_config = HfDataSourceConfig(
    name="mnist",
    split="train[:500]",
    exclude_keys={"id"},  # Exclude ID field
)

exclude_source = HFSource(exclude_config, rngs=nnx.Rngs(2))
```
...
Shuffling is essential for training ML models. HFSource supports both full shuffle and buffer-based shuffle.
...
source1 = HFSource(shuffle_config, rngs=nnx.Rngs(42, data_shuffle=123))
source2 = HFSource(shuffle_config, rngs=nnx.Rngs(42, data_shuffle=123))
...
```
Training pipeline:
  HFSource(mnist) -> Normalize -> RandomFlip -> Output
  Batch size: 64
```
...
flowchart TB
    ...
    subgraph Config["Configuration"]
        Cfg[HfDataSourceConfig<br/>name, split, shuffle<br/>include/exclude keys]
    end
...
large_config = HfDataSourceConfig(
    name="imagenet-1k",
    split="train",
    streaming=True,
    shuffle=True,
...
eval_config = HfDataSourceConfig(
    name="mnist",
    split="test",
    streaming=False,
    shuffle=False,
...
- **API Reference**: [HFSource Documentation](../../../sources/hf_source.md)
