...
This quick reference demonstrates loading datasets from TensorFlow Datasets (TFDS) using Datarax's `TFDSSource`. You'll load MNIST, apply transformations, and iterate through batched data using the standard Datarax pipeline API.

## What You'll Learn

1. Configure and create a `TFDSSource` with proper configuration
...
| PyTorch | Datarax |
|---------|---------|
| `torchvision.datasets.MNIST(train=True)` | `TFDSSource(TfdsDataSourceConfig(name="mnist", split="train"))` |
| `DataLoader(dataset, shuffle=True)` | `TFDSSource` with `shuffle=True` in config |
...
|--------------------|---------|
| `tfds.load('mnist', split='train')` | `TFDSSource(TfdsDataSourceConfig(name='mnist', split='train'))` |
...
**Terminal Output:**
```
Pipeline: TFDSSource(MNIST) -> Normalize -> Output
Batch size: 32
```
...
flowchart LR
    ...
    subgraph Source["TFDSSource"]
        Config[TfdsDataSourceConfig<br/>name, split, shuffle]
...
```
TFDSSource → Normalize → Batch → JAX Arrays
```
...
train_source = TFDSSource(train_config, rngs=nnx.Rngs(42, shuffle=42))
...
test_source = TFDSSource(test_config, rngs=nnx.Rngs(0))
...
dev_config = TfdsDataSourceConfig(
    name="mnist",
    split="train[:100]",
    shuffle=False,
)

dev_source = TFDSSource(dev_config, rngs=nnx.Rngs(0))
...
config = TfdsDataSourceConfig(
    ...
    shuffle=True,
    stochastic=True,
    stream_name="data_shuffle",  # Named stream
)

source = TFDSSource(config, rngs=nnx.Rngs(42, data_shuffle=123))
```
...
dev_config = TfdsDataSourceConfig(name="imagenet2012", split="train[:1000]")

# Production: Full dataset
prod_config = TfdsDataSourceConfig(name="imagenet2012", split="train")
```
...
- **API Reference**: [TFDSSource Documentation](../../../sources/tfds_source.md)
