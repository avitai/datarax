#!/usr/bin/env python3
"""Write the format-2 iterator fixture in an environment that neither floats nor drifts.

The fixture must be written by the releases that produced it (datarax 0.1.11 over substrax
0.1.9), which the project environment no longer holds, so the generator runs in an isolated
...
PRODUCERS = ("datarax==0.1.11", "substrax==0.1.9")
LOCKED = ("jax", "jaxlib", "flax")
GENERATOR = Path("scripts") / "make_format2_iterator_fixture.py"
...
def main(argv: list[str]) -> int:
    ...
    return subprocess.run(invocation, cwd=REPO_ROOT, check=False).returncode  # noqa: S603
