...
Usage:
    ...
    python scripts/jupytext_converter.py batch-nb-to-py examples/core/
...
''' + "=" * 80)
   ...
   Reference: Investigated during workshop examples refactoring.
...
def convert_py_to_nb(py_file: Path, verbose: bool = False) -> bool:
    ...
    if not is_valid:
        msg = (
            f"  Warning: Found {len(issues)} potential issue(s) that may cause conversion problems:"
        )
        print(msg)
        print()
        for issue in issues:
    ...
    print(f"  Converting {py_file.name} -> {nb_file.name}")
    ...
    if success:
        ...
        try:
            ...
            print(f"  Created {nb_file.name}")
            return True
        except Exception as e:
            print(f"  Created {nb_file.name} but failed to strip cell IDs: {e}")
...
def convert_nb_to_py(nb_file: Path, verbose: bool = False) -> bool:
    ...
    print(f"  Converting {nb_file.name} -> {py_file.name}")
    ...
    if success:
        ...
        print(f"  Failed to convert {nb_file.name}")
        return False
...
def sync_pair(file_path: Path, verbose: bool = False) -> bool:
    ...
    print(f"  Syncing {py_file.name} <-> {nb_file.name}")
...
def batch_convert_directory(
    ...
    if not source_files:
        print(f"  No {source_format} files found in {directory}")
        return 0, 0
...
def validate_sync(directory: Path, verbose: bool = False) -> tuple[int, int, int]:
    ...
    for py_file in py_files:
        ...
        if not nb_file.exists():
            print(f"  Missing notebook: {py_file} -> {nb_file}")
        ...
        try:
            ...
            if has_formats:
                ...
                if verbose:
                    print(f"  No jupytext pairing metadata: {py_file}")
