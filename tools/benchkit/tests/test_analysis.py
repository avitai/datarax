...
class TestRankTable:
    ...
    def test_delta_from_best_calculation(self):
        ...
        run = Run(
            points=[
                Point(
                    "CV-1/A", scenario="CV-1",
                    tags={"framework": "A"},
                    metrics={"throughput": Metric(100.0)},
                ),
                Point(
                    "CV-1/B", scenario="CV-1",
                    tags={"framework": "B"},
...
class TestAggregateScore:
    ...
    def test_two_frameworks_ranked_correctly(self):
        ...
        run = Run(
            points=[
                Point("S/A", "S", tags={"framework": "A"},
                      metrics={"throughput": Metric(100.0), "latency": Metric(10.0)}),
                Point("S/B", "S", tags={"framework": "B"},
        ...
        run = Run(
            points=[
                Point("S/A", "S", tags={"framework": "A"},
                      metrics={"throughput": Metric(100.0)}),
                Point("S/B", "S", tags={"framework": "B"},
                      metrics={"throughput": Metric(50.0)}),
                Point("S/C", "S", tags={"framework": "C"},
                      metrics={"throughput": Metric(75.0)}),
            ],
            metric_defs={
                "throughput": MetricDef("throughput", "elem/s", "higher"),
            },
