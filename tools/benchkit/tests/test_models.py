...
class TestRankEntry:
    ...
    def test_serde_round_trip(self):
        entry = RankEntry(
            label="Datarax", value=5000.0, rank=1,
            is_best=True, delta_from_best=0.0,
        )
        data = entry.to_dict()
...
class TestSignificanceResult:
    ...
    def test_serde_round_trip(self):
        sr = SignificanceResult(
            p_value=0.03, statistic=2.5, effect_size=0.8,
            significant=True, method="wilcoxon",
        )
        data = sr.to_dict()
...
class TestScalingLaw:
    def test_construction(self):
        sl = ScalingLaw(
            coefficient=1.5, exponent=0.9,
            r_squared=0.98, complexity="O(n)",
        )
...
class TestTrendPoint:
    def test_construction(self):
        tp = TrendPoint(
            run_id="abc123", timestamp=datetime(2026, 2, 10, 12, 0, 0),
            value=5000.0, commit="def456",
        )
    ...
    def test_optional_fields(self):
        tp = TrendPoint(
            run_id="abc123", timestamp=datetime(2026, 2, 10),
            value=5000.0,
        )
    ...
    def test_serde_round_trip(self):
        tp = TrendPoint(
            run_id="abc123", timestamp=datetime(2026, 2, 10, 12, 0, 0),
            value=5000.0, commit="def456", lower=4800.0, upper=5200.0,
        )
        data = tp.to_dict()
...
class TestTrendSeries:
    ...
    def test_empty_points(self):
        ts = TrendSeries(
            metric="latency", point_name="NLP-1",
            tags={}, points=[],
        )
        assert ts.points == []
        data = ts.to_dict()
