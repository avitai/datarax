...
def detect_regressions(
    run: Run, baseline: Run, threshold: float = 0.05,
) -> list[Regression]:
...
def bootstrap_ci(
    samples: list[float], confidence: float = 0.95, n_boot: int = 1000,
) -> tuple[float, float]:
...
def rank_table(
    run: Run, metric: str, group_by_tag: str = "framework",
) -> list[RankEntry]:
    ...
    # Sort by value
    sorted_items = sorted(
        groups.items(), key=lambda x: x[1], reverse=higher_is_better,
    )
...
def significance_test(
    ...
    if len(a) != len(b):
        raise ValueError(
            f"Paired test requires equal lengths: len(a)={len(a)}, len(b)={len(b)}"
        )
    ...
    try:
        ...
        if not non_zero:
            return SignificanceResult(
                p_value=1.0, statistic=0.0, effect_size=effect_size,
                significant=False, method="wilcoxon",
            )
...
def scaling_fit(sizes: list[float], values: list[float]) -> ScalingLaw:
    ...
    if len(sizes) != len(values):
        raise ValueError(
            f"Mismatched lengths: len(sizes)={len(sizes)}, len(values)={len(values)}"
        )
    ...
    if ss_xx == 0:
        ...
        return ScalingLaw(
            coefficient=math.exp(mean_y), exponent=0.0,
            r_squared=0.0, complexity="O(1)",
        )
    ...
    ss_res = sum(
        ...
        for x, y in zip(log_sizes, log_values)
    )
