...
@click.group()
def main() -> None:
    """benchkit — Benchmark analysis and W&B dashboard tool."""
...
def trend(data: str, metric: str, point_name: str, framework: str, n_runs: int | None) -> None:
    ...
    for tp in series.points:
        ...
        click.echo(
            f"  {tp.timestamp.strftime('%Y-%m-%d %H:%M')}  "
            f"{tp.value:.2f}{ci_str}{commit_str}"
        )
...
def summary(data: str, run_id: str) -> None:
    ...
    for scenario, points in scenarios.items():
        ...
        for point in points:
            ...
            metrics_str = ", ".join(
                f"{k}={v.value:.2f}" for k, v in point.metrics.items()
            )
