...
class WandBExporter(Exporter):
    ...
    def _log_rank_tables(self, run: Run) -> None:
        ...
        for metric_name in run.metric_defs:
            ...
            table_data = [
                [r.rank, r.label, r.value, r.is_best, r.delta_from_best]
                for r in ranks
            ]
            wandb.log({
    ...
    def _log_aggregate_scores(self, run: Run) -> None:
        ...
        primary_weights = {
            name: 1.0
            for name, md in run.metric_defs.items()
            if md.priority.value == "primary"
        }
    ...
    def _log_pareto_front(self, run: Run) -> None:
        ...
        if "throughput" not in run.metric_defs or "latency_p50" not in run.metric_defs:
            return
        front = pareto_front(
            run.points, "throughput", "latency_p50",
            ...
            "analysis/pareto_front": wandb.Table(
                ...
                data=[
                    [
                        ...
                        if "latency_p50" in p.metrics else "—",
                    ]
                    for p in front
                ],
            ),
    ...
    def export_trends(
        ...
        *,
        ...
        table_data = [
            [tp.timestamp.isoformat(), tp.value, tp.commit or ""]
            for tp in series.points
        ]
        wandb.log({
