...
1. Create multiple TFDSSource instances
...
mnist_config = TfdsDataSourceConfig(
    name="mnist",
...
mnist_source = TFDSSource(mnist_config, rngs=nnx.Rngs(42, shuffle=42))

# Fashion-MNIST Source - 10 fashion classes
fashion_config = TfdsDataSourceConfig(
    name="fashion_mnist",
    split="train[:2000]",
    shuffle=True,
    stochastic=True,
...
fashion_source = TFDSSource(fashion_config, rngs=nnx.Rngs(43, shuffle=43))
...
fashion_source2 = TFDSSource(fashion_config, rngs=nnx.Rngs(43, shuffle=43))
...
mnist_src = TFDSSource(
    TfdsDataSourceConfig(name="mnist", split="train[:1000]", shuffle=False),
    rngs=nnx.Rngs(0),
)
mnist_prep = ElementOperator(
    ElementOperatorConfig(stochastic=False),
