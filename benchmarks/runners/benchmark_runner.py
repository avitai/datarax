        ...
        for mod in scenarios:
            ...
            if not can_run_scenario(variant):
                mb = estimate_scenario_memory_mb(
                    variant.config.dataset_size, variant.config.element_shape,
                )
                print(f"SKIP {scenario_id}/{variant_name}: "
                      f"{mb:.0f} MB exceeds available memory", file=sys.stderr)
                continue

            try:
                result = self.run_scenario(
                    mod, adapter, variant_name, num_repetitions,
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--profile", default="ci_cpu", help="Hardware profile name",
    )
    parser.add_argument(
        "--output-dir", default="benchmark-data/reports",
    ...
    parser.add_argument(
        "--baselines-dir", default="benchmarks/baselines",
    ...
    if args.generate_baselines:
        ...
        saved = runner.generate_baselines(
            adapter, args.baselines_dir, args.repetitions,
        )
