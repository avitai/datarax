...
class FullRunner:
    ...
    def __init__(
        ...
        self._runner = BenchmarkRunner(
            output_dir=self.output_dir, hardware_profile=hardware_profile,
        )
    ...
    def run_comparative(
        ...
        if adapter_filter is not None:
            available = {
                ...
                if name in adapter_filter
            }

        # Apply scenario filter
        if scenario_filter is not None:
            scenarios = [
                mod for mod in scenarios if mod.SCENARIO_ID in scenario_filter
            ]
        ...
        for adapter_name, adapter_cls in available.items():
            ...
            for mod in scenarios:
                ...
                try:
                    ...
                    print(
                        f"  {scenario_id}/{variant_name}: "
                        f"{throughput:.0f} elem/s",
                        file=sys.stderr,
                    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", default="benchmark-data/reports/releases/v1.0",
        help="Output directory for results",
    )
    parser.add_argument(
        "--profile", default="ci_cpu",
