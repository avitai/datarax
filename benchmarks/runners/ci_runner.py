    ...
    for mod in scenarios:
        ...
        try:
            ...
            if not can_run_scenario(variant):
                print(f"SKIP {scenario_id}/{tier1_variant}: exceeds memory",
                      file=sys.stderr)
                continue
            result = run_scenario(
                adapter,
...
def generate_ci_report(
    ...
    # Summary
    failures = sum(
        1 for v in verdicts if v is not None and v.get("status") == "failure"
    )
    warnings = sum(
        1 for v in verdicts if v is not None and v.get("status") == "warning"
    )
    passes = sum(
        1 for v in verdicts if v is not None and v.get("status") == "pass"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--baselines-dir", default=str(_DEFAULT_BASELINES),
        help="Baselines directory (skip comparison if doesn't exist)",
    )
    parser.add_argument(
        "--output-dir", default=str(_DEFAULT_OUTPUT),
