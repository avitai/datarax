...
class ChartGenerator:
    ...
    def generate_all(
        self, formats: tuple[str, ...] = ("png", "svg"),
    ) -> list[Path]:
    ...
    def throughput_bars(self) -> Figure:
        ...
        for i, adapter in enumerate(adapters):
            ...
            ax.bar(
                x + offset, throughputs, bar_width,
                label=adapter, color=_get_color(adapter),
            )
    ...
    def throughput_radar(self, top_n: int = 3) -> Figure:
        ...
        top_adapters = sorted(
            adapter_avg, key=adapter_avg.get, reverse=True,
        )[:top_n]
        ...
        for adapter in adapters:
            ...
            ax.plot(
                angles, values, "o-",
                label=adapter, color=_get_color(adapter), linewidth=2,
            )
    ...
    def latency_cdf(self, scenario_id: str = "CV-1") -> Figure:
        ...
        for adapter_name, result in sorted(scenario_results.items()):
            ...
            ax.step(
                sorted_times, cdf,
                label=adapter_name, color=_get_color(adapter_name), linewidth=2,
            )
    ...
    def memory_waterfall(self) -> Figure:
        ...
        ax.bar(
            x, rss_values, color=[_get_color(a) for a in adapters],
        )
    ...
    def scaling_curves(self, dimension: str = "batch_size") -> Figure:
        ...
        for adapter_name, adapter_results in sorted(self._results.results.items()):
            ...
            if points:
                ...
                ax.plot(
                    xs, ys, "o-",
                    label=adapter_name, color=_get_color(adapter_name),
    ...
    def feature_heatmap(self) -> Figure:
        ...
        for i, adapter in enumerate(adapters):
            scenario_results = {
                r.scenario_id for r in self._results.results.get(adapter, [])
            }
