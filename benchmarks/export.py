    ...
    return benchkit.Run(
        ...
        metric_defs={
            "throughput": benchkit.MetricDef(
                ...
                group="Throughput", priority=benchkit.MetricPriority.PRIMARY,
            ),
            "latency_p50": benchkit.MetricDef(
                "latency_p50", "ms", "lower",
                group="Latency", priority=benchkit.MetricPriority.PRIMARY,
            ),
            "latency_p95": benchkit.MetricDef(
                "latency_p95", "ms", "lower", group="Latency",
            ),
            "latency_p99": benchkit.MetricDef(
                "latency_p99", "ms", "lower", group="Latency",
            ),
            "peak_memory": benchkit.MetricDef(
                "peak_memory", "MB", "lower",
                group="Memory", priority=benchkit.MetricPriority.PRIMARY,
            ),
            "gpu_memory": benchkit.MetricDef(
                "gpu_memory", "MB", "lower", group="Memory",
            ),
        },
    )
...
class FullExporter:
    ...
    def _log_charts(
        self, comparative: ComparativeResults, chart_dir: Path | None,
    ) -> None:
        chart_gen = ChartGenerator(
            comparative, chart_dir or Path("benchmark-data/charts"),
        )
        chart_methods = [
    ...
    def _log_gaps(self, comparative: ComparativeResults) -> None:
        ...
        if gaps:
            self._exporter.log_extra_tables({
                "analysis/gaps": (
                    ["Priority", "Scenario", "Gap Ratio", "Framework", "Severity", "Mitigation"],
                    [
                        [g.priority, g.scenario_id, f"{g.gap_ratio:.1f}x",
                         g.top_alternative, g.severity, g.mitigation]
                        for g in gaps
                    ],
                ),
            })
    ...
    def _upload_results_artifact(
        self, results_dir: Path, run: benchkit.Run,
    ) -> None:
        ...
        artifact = wandb.Artifact(name=name, type="benchmark-results", metadata={
