...
def _make_data_generator(
    dataset_size: int, max_len: int, seed: int = DEFAULT_SEED
) -> callable:
    """Create a lazy data generator for padded variable-length sequences."""

    def generate() -> dict[str, Any]:
        gen = SyntheticDataGenerator(seed=seed)
        var_seqs = gen.variable_length_tokens(
            dataset_size, min_len=10, max_len=max_len
        )
    ...
    for name, spec in _VARIANT_SPECS.items():
        ...
        variants[name] = ScenarioVariant(
            ...
            data_generator=_make_data_generator(
                spec["dataset_size"], spec["max_len"]
            ),
        )
    return variants
