...
def _make_result(
    framework: str = "Datarax", scenario_id: str = "CV-1",
) -> BenchmarkResult:
...
class TestComparativeResults:
    ...
    def test_save_and_load_roundtrip(self, tmp_path: Path):
        """ComparativeResults must serialize to JSON and load back."""
...
def _setup_mocks(adapter_names, scenario_ids):
    ...
    for sid in scenario_ids:
        config = ScenarioConfig(
            scenario_id=sid, dataset_size=640,
            element_shape=(32, 32, 3), batch_size=32,
            transforms=[], seed=42, extra={"variant_name": "small"},
        )
...
class TestFullRunner:
    ...
    def _run_with_mocks(
        self, tmp_path, adapter_names, scenario_ids,
        scenario_filter=None, adapter_filter=None,
    ):
        ...
        def mock_run_scenario(
            adapter, variant,
            num_batches=50, warmup_batches=5, num_repetitions=5,
        ):
    ...
    def test_cli_platform_flag(self, tmp_path: Path):
        """FullRunner must accept platform parameter."""
        from benchmarks.runners.full_runner import FullRunner
