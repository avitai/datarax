...
class GapDetector:
    ...
    def detect(self) -> list[PerformanceGap]:
        ...
        for scenario_id in self._results.all_scenario_ids:
            ...
            severity = (
                "action_required" if ratio >= self._action else "warning"
            )
            priority = _PRIORITY_MAP.get(scenario_id, "P5")
            mitigation = _MITIGATION_MAP.get(
                scenario_id, "Profile and optimize hot path",
            )
    ...
    def generate_backlog(self, output_path: Path) -> None:
        ...
        if not gaps:
            ...
            for gap in gaps:
                ...
                lines.append(
                    f"- **Datarax:** {gap.datarax_throughput:.0f} elem/s"
                )
                lines.append(
                    f"- **{gap.top_alternative}:** "
                    f"{gap.alternative_throughput:.0f} elem/s"
                )
