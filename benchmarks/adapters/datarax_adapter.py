...
# Transform name -> function mapping
...
class DataraxAdapter(BenchmarkAdapter):
    ...
    def _create_operator(
        self, transform_name: str, rngs: nnx.Rngs,
    ) -> ElementOperator:
    ...
    def _create_source(
        self, config: ScenarioConfig, data: Any, rngs: nnx.Rngs,
    ) -> Any:
    ...
    def setup(self, config: ScenarioConfig, data: Any) -> None:
        ...
        for transform_name in config.transforms:
            if transform_name in _TRANSFORM_FNS:
    ...
    def warmup(self, num_batches: int = 3) -> None:
        ...
        for i, batch in enumerate(self._pipeline):
            if i >= num_batches:
                break
            for leaf in jax.tree.leaves(batch.get_data()):
    ...
    def supported_scenarios(self) -> set[str]:
        """All 25 benchmark scenarios supported by Datarax."""
        return {
            "CV-1", "CV-2", "CV-3", "CV-4",
            "NLP-1", "NLP-2",
            "TAB-1", "TAB-2",
