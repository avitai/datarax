...
class TestExportToBenchkit:
    ...
    def test_throughput_mapped(self, sample_comparative):
        run = export_to_benchkit(sample_comparative)
        datarax_point = next(
            p for p in run.points if p.tags["framework"] == "Datarax"
        )
    ...
    def test_memory_absent_when_no_resources(self, sample_comparative):
        run = export_to_benchkit(sample_comparative)
        grain_point = next(
            p for p in run.points if p.tags["framework"] == "Grain"
        )
...
class TestFullExporter:
    ...
    def test_full_export_delegates_to_wandb_exporter(
        self, sample_comparative, sample_run, mock_store, mock_wandb_exporter,
    ):
    ...
    def test_full_export_logs_charts(
        self, sample_comparative, sample_run, mock_store, mock_wandb_exporter, tmp_path,
    ):
    ...
    def test_full_export_with_gap_scenario(
        self, mock_store, mock_wandb_exporter,
    ):
        ...
        results = {
            "Datarax": [
                BenchmarkResult(
                    framework="Datarax", scenario_id="CV-1", variant="small",
                    timing=timing_slow, resources=None,
                    environment={"cpu": "test"}, config={"batch_size": 32},
                ),
            ],
            "Grain": [
                BenchmarkResult(
                    framework="Grain", scenario_id="CV-1", variant="small",
                    timing=timing_fast, resources=None,
                    environment={"cpu": "test"}, config={"batch_size": 32},
                ),
            ],
        }
